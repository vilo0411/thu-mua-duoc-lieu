...
3. `.agents/workflows/drafting.md` — viết bản nháp Markdown đầy đủ. **Dừng chờ xác nhận draft** (dùng `.agents/workflows/revise.md` nếu cần sửa).
4. `.agents/workflows/approve.md` — chạy QA (vai Quality Guardian) và chuyển `2-draft` → `3-finalized` → chuyển đổi sang JSON và publish vào `content/wiki/` hoặc `content/wiki-hub/` (hoặc `content/_drafts/wiki-yte/` nếu YMYL).
